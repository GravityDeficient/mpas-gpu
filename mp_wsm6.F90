...
!
 ...
 kdim = kte-kts+1
!
!--- OpenACC Stage 2 fix: Stage 0's aggressive data region (create of all
!    local arrays as GPU-only) is incoherent with a partial port where
!    CPU code still reads many of those arrays.  Stage 2 kernels write
!    to cpm/xl/delz_tmp/den_tmp/tstepsnow/tstepgraup which CPU code
!    subsequently reads — under the create() semantics CPU gets
!    uninitialized memory → NaN surface pressure → blow-up at ~step 600.
!    Removing the data region; each kernel now does its own implicit
!    copy.  Higher per-kernel overhead, but correctness first.
!    Rebuild a proper data region once all kernels are in place and
!    the full read/write flow is known.
!
!----------------------------------------------------------------
! paddint 0 for negative values generated by dynamics
!
! OpenACC Stage 1: first GPU kernel.  qc/qr/qi/qs/qg are inout arguments
! not in the surrounding data region, so OpenACC will do an implicit copy
! for this kernel.  That overhead is non-zero but acceptable as a starting
! point.  Later stages may migrate the data region up to cover them.
!
!----------------------------------------------------------------
! Optimization Stage A1: fused pad-zero + cpm/xl + delz/den into one
! kernel (was 2 parallel regions with 3 loop nests → 1 region, 1 loop).
!
...
!
! OpenACC Stage 2b: 1D column-loop kernel.  The present() checks for
! the optional arguments resolve at subroutine entry (invariant across
! loop iterations) so they are safe inside a parallel region.
!
!$acc parallel vector_length(32)
...
!
! OpenACC Stage 3a: 1D init of mstep, flgcld on GPU.
!
! Optimization Stage A2: fused mstep/flgcld init + denfac into one
! kernel. 1D work guarded by if(k==kts) to avoid redundant writes.
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2)
   do k = kts, kte
...
!  qsat(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
   ...
   xbi=xai+hsub/(rv*ttp)
!
! OpenACC Stage 3a: saturation mixing ratio + relative humidity for
! both liquid (1) and ice (2) reference states.  The per-cell branch
! on `if(t < ttp)` is natural warp divergence but Blackwell ITS
! handles it; branch cost is small relative to the exp/log math.
!
!$acc parallel vector_length(32)
...
!
!
! OpenACC Stage 3b (first kernel): zero-init the 27 process-rate arrays
! plus the fall/falk/falkc scratch and xni default.  Pure pointwise
! write-only pattern -- ideal for GPU.
!
! Optimization Stage B: fused process-rate zero-init + xni + qrs_tmp
! packing into one kernel (was 3 parallel regions → 1).
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) private(temp)
   do k = kts, kte
     do i = its, ite
       ...
       fallc(i,k) = 0.
       falkc(i,k) = 0.
! xni: ice crystal number [HDC 5c]
       temp = (den(i,k)*max(qi(i,k),qmin))
       temp = sqrt(sqrt(temp*temp*temp))
       xni(i,k) = min(max(5.38e7*temp,1.e3),1.e6)
! qrs_tmp packing for slope_wsm6
...
! Optimization Stage C1: fused post-sed unpack + 1D fall + qrs_tmp repack
! (was 3 parallel regions → 1). 1D surface work guarded by if(k==kts).
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2)
   do k = kts, kte
...
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) private(supcol,xlf,coeres)
   do k = kte, kts, -1
     do i = its, ite
       supcol = t0c-t(i,k)
...
!---------------------------------------------------------------
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) private(xmi,diameter)
   do k = kte, kts, -1
     do i = its, ite
       if(qi(i,k).le.0.) then
         work1c(i,k) = 0.
...
!$acc end parallel
   call nislfv_rain_plm(idim,kdim,den_tmp,denfac,t,delz_tmp,work1c,denqci,  &
                        delqi,dtcld,1,0)
! Optimization Stage C2: fused ice post-sed unpack + fallc surface (was 2 → 1).
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2)
   do k = kts, kte
     do i = its, ite
       ...
       if(k.eq.kts) fallc(i,1) = delqi(i)/delz(i,1)/dtcld
     enddo
   enddo
!$acc end parallel
!
...
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) &
!$acc   private(supcol,xlf,pfrzdtc,supcolt,temp,pfrzdtr)
   do k = kts, kte
     do i = its, ite
       supcol = t0c-t(i,k)
       xlf = xls-xl(i,k)
...
! Optimization Stage D: qrs_tmp repack fused into freezing/melting kernel
       qrs_tmp(i,k,1) = qr(i,k)
       qrs_tmp(i,k,2) = qs(i,k)
       qrs_tmp(i,k,3) = qg(i,k)
     enddo
...
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) private(supsat,satdt,coeres)
   do k = kts, kte
     do i = its, ite
...
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) &
!$acc   private(supcol,supsat,satdt,ifsat,temp,eacrs,xmi,diameter) &
!$acc   private(vt2i,vt2r,vt2s,vt2g,vt2ave,acrfac,egi,xlf) &
!$acc   private(supice,coeres,xni0,roqi0,qimax,alpha2)
   do k = kts, kte
     do i = its, ite
       supcol = t0c-t(i,k)
       n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
...
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) &
!$acc   private(delta2,delta3,value,source,factor,xlf,xlwork2)
   do k = kts, kte
     do i = its, ite
!
       delta2=0.
...
!
!
!--- (Stage 0 data region removed in Stage 2 fix; see note near line 437)
!----------------------------------------------------------------
! CCPP checks:
!
...
!
! OpenACC Stage 4: entire body of slope_wsm6 wrapped in one parallel region.
! Called CPU-to-CPU from mp_wsm6_run; internally launches one GPU kernel.
! Statement functions lamdar/lamdas/lamdag are inlined by nvfortran at call
! sites, pulling the module-level constants pidn0r/pidn0s/pidn0g as
! implicit firstprivate.  Local scalars supcol,x,y,z go in private().
!
!$acc parallel vector_length(32)
!$acc loop gang vector collapse(2) private(supcol,x,y,z)
 do k = kts, kte
   do i = its, ite
     supcol = t0c-t(i,k)
!---------------------------------------------------------------
...
!$acc   private(dz,ww,qq,wd,wa,was,den,denfac,tk) &
!$acc   private(qn,qr,tmp,tmp1,tmp2,tmp3) &
!$acc   private(wi,zi,za,dza,qa,qmi,qpi) &
!$acc   private(k,n,m,kk,kb,kt) &
!$acc   private(tl,tl2,qql,dql,qqd,th,th2,qqh,dqh) &
!$acc   private(zsum,qsum,dim,dip,c1,con1,fa1,fa2) &
!$acc   private(allold,allnew,zz,dzamin,cflmax,decfl)
 i_loop: do i=1,im
    precip(i) = 0.0
    dz(:) = dzl(i,:)
    qq(:) = rql(i,:)
...
! then back to use mean terminal velocity
    if( n.le.iter ) then
!      (inlined slope_rain for GPU — avoids !$acc routine seq on module vars)
       do k=1,km
         if(qr(k).le.qcrmin) then
           tmp(k) = rslopermax; tmp1(k) = rsloperbmax
           tmp2(k) = rsloper2max; tmp3(k) = rsloper3max
         else
           tmp(k) = 1./sqrt(sqrt(pidn0r/(qr(k)*den(k))))
           tmp1(k) = exp(log(tmp(k))*(bvtr))
           tmp2(k) = tmp(k)*tmp(k); tmp3(k) = tmp2(k)*tmp(k)
         endif
         wa(k) = pvtr*tmp1(k)*denfac(k)
         if(qr(k).le.0.0) wa(k) = 0.0
       enddo
...
!---- local variables and arrays:
 ...
 real(kind=kind_phys):: precip_local
 real(kind=kind_phys):: supcol,n0sfac_local

!-----------------------------------------------------------------------------------------------------------------
!$acc parallel loop gang vector &
!$acc   private(dz,ww,qq,qq2,wd,wa,wa2,was,den,denfac,tk) &
!$acc   private(qn,qr,qr2,tmp,tmp1,tmp2,tmp3) &
!$acc   private(wi,zi,za,dza,qa,qa2,qmi,qpi) &
!$acc   private(k,ist,n,m,kk,kb,kt) &
!$acc   private(tl,tl2,qql,dql,qqd,th,th2,qqh,dqh) &
!$acc   private(zsum,qsum,dim,dip,c1,con1,fa1,fa2) &
!$acc   private(allold,allnew,zz,dzamin,cflmax,decfl,precip_local) &
!$acc   private(supcol,n0sfac_local)
 i_loop: do i=1,im
    precip1(i) = 0.0
    precip2(i) = 0.0
    dz(:) = dzl(i,:)
...
! then back to use mean terminal velocity
    if( n.le.iter ) then
!      (inlined slope_snow for GPU)
       do k=1,km
         supcol = 273.15-tk(k)
         n0sfac_local = max(min(exp(alpha*supcol),n0smax/n0s),1.)
         if(qr(k).le.qcrmin) then
           tmp(k) = rslopesmax; tmp1(k) = rslopesbmax
           tmp2(k) = rslopes2max; tmp3(k) = rslopes3max
         else
           tmp(k) = 1./sqrt(sqrt(pidn0s*n0sfac_local/(qr(k)*den(k))))
           tmp1(k) = exp(log(tmp(k))*(bvts))
           tmp2(k) = tmp(k)*tmp(k); tmp3(k) = tmp2(k)*tmp(k)
         endif
         wa(k) = pvts*tmp1(k)*denfac(k)
         if(qr(k).le.0.0) wa(k) = 0.0
       enddo
!      (inlined slope_graup for GPU)
       do k=1,km
         if(qr2(k).le.qcrmin) then
           tmp(k) = rslopegmax; tmp1(k) = rslopegbmax
           tmp2(k) = rslopeg2max; tmp3(k) = rslopeg3max
         else
           tmp(k) = 1./sqrt(sqrt(pidn0g/(qr2(k)*den(k))))
           tmp1(k) = exp(log(tmp(k))*(bvtg))
           tmp2(k) = tmp(k)*tmp(k); tmp3(k) = tmp2(k)*tmp(k)
         endif
         wa2(k) = pvtg*tmp1(k)*denfac(k)
         if(qr2(k).le.0.0) wa2(k) = 0.0
       enddo
       do k = 1, km
...
! rain out
       sum_precip: do k=1,km
          if( za(k).lt.0.0 .and. za(k+1).lt.0.0 ) then
             precip_local = precip_local + qa(k)*dza(k)
             cycle sum_precip
          else if ( za(k).lt.0.0 .and. za(k+1).ge.0.0 ) then
             precip_local = precip_local + qa(k)*(0.0-za(k))
             exit sum_precip
          endif
          exit sum_precip
       enddo sum_precip
...
! replace the new values
       if(ist.eq.1) then
         rql(i,:) = qn(:)
         precip1(i) = precip_local
       else
         rql2(i,:) = qn(:)
         precip2(i) = precip_local
       endif
    enddo ist_loop
