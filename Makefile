...
#
# RRTMGP + netcdf-fortran (used by src/core_atmosphere/physics/physics_rrtmgp).
# RRTMGP_PATH defaults to the in-tree build of the rte-rrtmgp submodule --
# produced by running:
#   git submodule update --init src/external/rte-rrtmgp
#   ./scripts/build-rte-rrtmgp.sh
# NETCDFF_PATH must point at an nvfortran-compatible netcdf-fortran install
# (static libs, --disable-shared, linking system libnetcdf). Defaults to the
# Huginn scratch install; fresh hosts need to provide their own.
# Override either on the command line or via environment:
#   make nvhpc CORE=atmosphere OPENACC=true RRTMGP_PATH=... NETCDFF_PATH=...
#
RRTMGP_PATH  ?= $(CURDIR)/src/external/rte-rrtmgp/install
NETCDFF_PATH ?= /home/admin/rrtmgp-probe/ncf-nvhpc
export RRTMGP_PATH NETCDFF_PATH

CPPINCLUDES += -I$(RRTMGP_PATH)/include -I$(NETCDFF_PATH)/include
FCINCLUDES  += -I$(RRTMGP_PATH)/include -I$(NETCDFF_PATH)/include
# NB: -lrrtmgp must precede -lrte (uses librte). netcdf-fortran after rrtmgp
# because mo_simple_netcdf.F90 pulls in the netcdf module.
LIBS += -L$(RRTMGP_PATH)/lib -lrrtmgp -lrte -L$(NETCDFF_PATH)/lib -lnetcdff
